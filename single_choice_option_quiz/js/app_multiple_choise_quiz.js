...
function digitFormateHandler(singledigitfiger) {
...
//   Array.from(
//     Option.parentElement.parentElement.querySelectorAll('input[type="radio"]')
//   ).forEach((inp) => {
//     let values = [];
//     if (inp.checked) {
...
function checkedOptionChecker(inputOptions) {
  if (inputOptions.checked) {
    ...
    if (isRightAnswer) {
      ...
      document.querySelector(
        "#wronganswer"
      ).innerHTML = `Wrong Answer : ${digitFormateHandler(wrongAnswer)} `;
